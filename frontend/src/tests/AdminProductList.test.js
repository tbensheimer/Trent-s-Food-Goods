...
it('Should render AdminProductList component properly', async () => {
    ...
    await act(async() => render(<Provider store={store}><BrowserRouter><AdminProductList /></BrowserRouter></Provider>));

    const products = screen.getAllByTestId("product");
    const pageLinks = screen.getAllByTestId('page-links');
    const createProductBtn = screen.getByText("Create New Product");
